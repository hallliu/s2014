...
If we estimate the two means as $\conj{x_1}$ and $\conj{x_2}$, using the expression from (a), the decision boundary would be 
{
    \nc{\cxi}{\conj{x_1}}
    \nc{\cxii}{\conj{x_2}}
    $\frac{\cxii^2-\cxi^2}{2(\cxii-\cxi)}$
}

Code for the computation of error rates from the samples is attached, using the expression from (b). A histogram follows.

\includegraphics[width=0.6\textwidth]{hw1_files/p3.png}
